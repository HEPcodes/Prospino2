...
C ======================================================================
      ...
      SUBROUTINE 
     &  INTEGGG(IFAST,RESLO_NG,ERRLO_NG,RESLO,ERRLO,RESNLO,ERRNLO)
      IMPLICIT REAL*8 (A-H,M-Z)
      IMPLICIT INTEGER (I,J)
      integer  ivegas(1:4)
      COMMON/CONST1/S,ENERGY,ALPHAS,MS,MG,MT
      ...
      EXTERNAL SIGLOGG, SIGNLOGG, SIGLOGG_NG
      ...
      idim = 3
      IF (ING.EQ.1)
     &   call INTEG(SIGLOGG_NG,idim,ivegas,ifast,reslo_ng,errlo_ng)
      IF (IDG.EQ.1)
     &   call INTEG(SIGLOGG,   idim,ivegas,ifast,reslo,   errlo   )
 
      IF ( (IONLYLO.EQ.0).AND.(IDG.EQ.1) ) THEN
         CALL ININLO(ivegas)
         idim = 5 
         idum = 0 
         call INTEG(SIGNLOGG,idim,ivegas,idum,resnlo,errnlo)
      ELSE 
         RESNLO = 0D0
         ERRNLO = 0D0
      END IF
...
C***  THE CROSS-SECTION IN LO for free squark masses
      ...
      REAL*8 FUNCTION SIGLOGG_NG(VAR)
      IMPLICIT REAL*8 (A-H,M-Z)
      IMPLICIT INTEGER (I,J)
      DIMENSION VAR(3)
      COMMON/CONST1/S,ENERGY,ALPHAS,MS,MG,MT
      ...
      CALL DVEGLO_GG(VAR)
      IF (ITOTAL.EQ.0) THEN
         print*, " SIGLOGG_NG: distributions not yet implemented "
         stop
      ELSE
         CALL DEFSCAGG                                             ! define scales
         CALL DTOLOGG                                              ! define 2->2 kinematics
         SIGLOGG1 = FGGGGLO()                                      ! no squark mass involved
         SIGLOGG2 = FGGQBLO_NG()
      END IF
      SIGLOGG_NG =  SUMLOGG(SIGLOGG1,SIGLOGG2,IFLAVOR)
...
ctp only including bottom densities when they are actually set
      IF (ICOLL.EQ.0) THEN
         DO I= 1,5
            PDF(I) = XPDF( I) * YPDF( I) + XPDF(-I) * YPDF(-I) 
         END DO
      ...
      FGGQBLO_NG = 
     +     PDF(1) * PREF*DGGQBB_NG(ALPHAS,SHAT,TGHAT,
     +                             MSQ_COMMON(-1),MSQ_COMMON(1),MG) 
     +   + PDF(2) * PREF*DGGQBB_NG(ALPHAS,SHAT,TGHAT,
     +                             MSQ_COMMON(-2),MSQ_COMMON(2),MG) 
     +   + PDF(3) * PREF*DGGQBB_NG(ALPHAS,SHAT,TGHAT,
     +                             MSQ_COMMON(-3),MSQ_COMMON(3),MG) 
     +   + PDF(4) * PREF*DGGQBB_NG(ALPHAS,SHAT,TGHAT,
     +                             MSQ_COMMON(-4),MSQ_COMMON(4),MG) 
     +   + PDF(5) * PREF*DGGQBB_NG(ALPHAS,SHAT,TGHAT,
     +                             MSQ_COMMON(-5),MSQ_COMMON(5),MG) 
     ...
     +                             MS,MS,MG) 
      ELSE 
         PRINT*," FGGQBLO_NG: i_ngtest not set ",I_NGTEST_COMMON
         CALL HARD_STOP
      END IF
