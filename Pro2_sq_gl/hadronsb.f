...
C ======================================================================
      ...
      SUBROUTINE 
     &  INTEGSB(IFAST,RESLO_NG,ERRLO_NG,RESLO,ERRLO,RESNLO,ERRNLO)
      IMPLICIT REAL*8 (A-H,M-Z)
      IMPLICIT INTEGER (I,J)
      integer  ivegas(1:4)
      COMMON/CONST1/S,ENERGY,ALPHAS,MS,MG,MT
      ...
      EXTERNAL SIGLOSB, SIGNLOSB, SIGLOSB_NG
      ...
      idim = 3
      IF (ING.EQ.1)
     &   call INTEG(SIGLOSB_NG,idim,ivegas,ifast,reslo_ng,errlo_ng)
      IF (IDG.EQ.1)
     &   call INTEG(SIGLOSB,   idim,ivegas,ifast,reslo,   errlo   )

      IF ( (IONLYLO.EQ.0).AND.(IDG.EQ.1) ) THEN
         CALL ININLO(ivegas)
         idim = 5 
         idum = 0
         call INTEG(SIGNLOSB,idim,ivegas,idum,resnlo,errnlo)
      ELSE 
         RESNLO = 0D0
         ERRNLO = 0D0
      END IF
...
C***  THE CROSS-SECTION IN LO for free squark masses
      ...
      REAL*8 FUNCTION SIGLOSB_NG(VAR)
      IMPLICIT REAL*8 (A-H,M-Z)
      IMPLICIT INTEGER (I,J)
      DIMENSION VAR(3)
      COMMON/CONST1/S,ENERGY,ALPHAS,MS,MG,MT
      ...
      CALL DVEGLO_SB(VAR)
      IF (ITOTAL.EQ.0) THEN
         print*, " SIGLOSB_NG: distributions not yet implemented "
         stop
      ELSE
         CALL DEFSCASB
         CALL DTOLOSB
...
C***  FILLS UP THE COMMON BLOCK TOLOSB AND PDLOSB
      ...
      COMMON/TOLOSB_NG/MS1_NG,MS2_NG
     ...
     &              ,I_NGTEST_COMMON
      ...
      ELSE IF (I_NGTEST_COMMON.EQ.1) THEN
         ...
         MS2_NG = MS
      ELSE
         PRINT*," DTOLOSB: i_ngtest not set ",I_NGTEST_COMMON
         CALL HARD_STOP
      END IF 
...
ctp      BETA_NG = SQRT(1D0 -4*MSX**2/SHAT_NG)
     ...
     &         * (1D0-(MS1_NG-MS2_NG)**2/SHAT_NG)
      ...
      ELSE
         PRINT*, " DTOLOSB: PROBLEM WITH BETA ",BETA_NG
      END IF
...
C***  CALCULATES THE TOTAL CROSS-SECTION IN LO FOR G G
     ...
     &              ,I_NGTEST_COMMON
      ...
      IF (ISQUARK1_COMMON .NE. ISQUARK2_COMMON) THEN 
         FSBGGLO_NG = 0.D0
         RETURN
      END IF
...
ctp parton densities in CteQ conventions
      ...
      FSBGGLO_NG = XPDF(0)*YPDF(0)
     +   * PREF_NG * DSBGGB_NG(ALPHAS,SHAT_NG,T1HAT_NG,MS1_NG,MG) 
...
ctp pdf has to be an argument because initial state summed over in s channel
      FSBQBLO_NG = PREF_NG
     +           * DSBQBB_NG(ALPHAS,SHAT_NG,T1HAT_NG,MS1_NG,MS2_NG,MG
     +                      ,ISQUARK1_COMMON,ISQUARK2_COMMON,PDF) 
